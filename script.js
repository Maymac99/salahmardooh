...
(function ($) {
  async function loadSharedLayout() {
    ...
    try {
        // Load general HTML template
        const res = await fetch("index.html");
        const html = await res.text();

        // Inject into local page
        container.innerHTML = html;

        // After injecting layout → load data
        loadPageData();
    } catch (err) {
        console.error("Error loading shared layout:", err);
    }
}

  const DRAWN_KEY = () => `drawnIds_${pageId}`;
  ...
  $(document)
    .keydown(function (e) {
      if (e.keyCode === 91 || e.keyCode === 17) ctrlPressed = true;
        ...
        alert(`Draw reset for: ${pageId.toUpperCase()}`);
      }
    })
    .keyup(function (e) {
  ...
  async function loadMembers() {
    ...
    try {
      ...
      allMembers = Array.isArray(data) ? data.map((m, i) => ({ ...m, _id: i })) : [];
    ...
    for (let slotId = 1; slotId <= finalWinners.length; slotId++) {
      ...
      // Display with status (on separate line if in General)
        ...
        if (currentStatus === "General") {
          displayName = `${target['Full Name']}<br><small>${target.Status}</small>`;
        } else {
          displayName = target['Full Name'];
        }
  ...
  $(document).ready(async function () {
    ...
    function filterMembersByStatus() {
      currentStatus = $("#status-select").val();

      if (currentStatus === "General") {
        members = allMembers
      } else {
        members = allMembers.filter(
          (m) => m.Status && m.Status.trim() === currentStatus
        );
      }
    ...
    $("#status-select").on("change", filterMembersByStatus);
    ...
    $("#drawButton").on("click", async function () {
      ...
      // Update modal title
      $("#resultsModal h2").text(`🎉 LUCKY WINNERS — ${currentStatus.toUpperCase()} 🎉`);
